...
struct key
{
  uint8_t normalised; // number between 0-255 where 0 is unpressed and 255 is
                      // fully depressed
  ...
  int has_value_changed; // If the value hasnt changed for a few frames and the
                         // key isnt pressed we can reset the min value
...
void setup()
{
  ...
  for (int i = 0; i < KEY_COUNT; i++)
  {
    ...
    keys[i].min_real = keys[i].real; // set the min value to the value at
                                     // startup
  }
...
void setpins(const uint8_t value)
{
  ...
  digitalWrite(MTP_BIN_PIN_5, ~(bool(value & (0b00110000)))); // 000 the value
                                                              // when value =
                                                              // false
...
void process_hid()
{
  ...
  for (int i = 0; i < KEY_COUNT; i++)
  {
    ...
    {
      if (keys[i].max_real != keys[i].min_real &&
          keys[i].max_real - keys[i].min_real > 50) // only set modifier if
        keys[i].factor = (float)(NORMALISED_ADC_VAL)(-1) /
    ...
    if (keys[i].active_state)
    {
          ...
          keys[i].normalised < +previous) // check if unpressed and key at top
      {
        ...
        change_modifier();
          ...
          keys[i].normalised <
              MAX_NORMALISED_ADC_VAL - bounds_checker) // check if direction of
                                                       // keystroke changed in
                                                       // middle of stroke
      {
        keys[i].active_state = false;
      ...
      else if (keys[i].normalised > previous) // check if key travelling
                                              // downwards
      {
        keycodes[count++] = keys[i].keycode;
      }
    ...
    else // if key not already pressed
    {
      ...
      else if (keys[i].normalised > previous) // if the keystroke is going
                                              // downwards then set the value to
                                              // previous so that the distance
                                              // gets bigger if it keeps going
                                              // down
      {
        keys[i].normalised = previous;
      }
  ...
  if (count)
  {
    ...
    uint8_t const modifier = 0; // modifier keys stored in array of 1 bit
                                // numbers
